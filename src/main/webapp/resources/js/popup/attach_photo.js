    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
			...
			aFileList.push('	'); 
			aFileList.push('	<select name="imgReSize'+nImageInfoCnt+'" id="imgReSize'+nImageInfoCnt+'">'); 
			aFileList.push('	<option value="0" selected>원본크기</option>'); 
			aFileList.push('	<option value="320">320px</option>'); 
			aFileList.push('	<option value="400">400px</option>'); 
			aFileList.push('	<option value="480">480px</option>'); 
			aFileList.push('	<option value="550">550px</option>'); 
			aFileList.push('	<option value="650">650px</option>'); 
			aFileList.push('	<option value="740">740px</option>'); 
			aFileList.push('	<option value="900">900px</option>'); 
			aFileList.push('	</select>');
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/gun/smartEditorFileUploader.jsp'; 	//upload URL
    	
    	//파일을 하나씩 보내고, 결과를 받음.
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		tempFile = htImageInfo['img'+j];
    		//셀렉트 박스에서 선택 이미지 사이즈 값 
            reSizeObject = document.getElementById('imgReSize' + j);
            reSize = reSizeObject.options[reSizeObject.selectedIndex].value;
            
    		try{
	    		if(!!tempFile){
	    			//Ajax통신하는 부분. 파일과 업로더할 url을 전달한다.
	    			//reSize 파라미터 추가
	    			callAjaxForHTML5(tempFile,sUploadURL, reSize);
	    			k += 1;
	    		}
	    	}catch(e){}
    		tempFile = null;
     ...
     * 
     */
    function callAjaxForHTML5 (tempFile, sUploadURL, reSize){
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					if(sResString.indexOf("NOTALLOW_") > -1){
						...
						//reSize 파라미터 추가
						makeArrayFromString(res._response.responseText+"&reSize="+reSize);
					}
				}
			},
			timeout : 3,
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/gun/singleFileUploader.jsp',	//샘플 URL입니다.
 	        sCallback : '/gun/smartEditor/photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	error : function(oCustomEvent) {
 	    		...
 	    		alert('error는 : ' + oCustomEvent.htResult.errstr);
 	    	}
 	    });
 	}
