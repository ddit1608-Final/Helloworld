...
<script type="text/javascript">
	function result(zipNum, sido, gugun, dong) {
		window.opener.document.getElementById('addr_zipnum').value =zipNum;
		window.opener.document.getElementById('mem_addr').value =sido + " " + gugun + " " + dong;
		...
		<table id="zipcode" class="table table-striped">
			...
			<tbody>
				<c:forEach items="${addressList}" var="addressVO">
					<tr>
						<td>${addressVO.addr_zipNum}</td>
						<td><a href="#"
							onclick="return result('${addressVO.addr_zipNum}'
,'${addressVO.addr_sido}', '${addressVO.addr_gugun}','${addressVO.addr_dong}')">
								${addressVO.addr_sido} ${addressVO.addr_gugun} ${addressVO.addr_dong} </a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
