		    ...
		    <div class="modal-dialog">
		      ...
		      <div class="modal-content">
		        ...
		        <div class="modal-body">
		          <p>이메일:${dogBoardVO.mem_mail} <a href="#">메일보내기</a></p>
		          <p>연락처:01*-**${dogBoardVO.mem_phn}*-**** <a href="#">문자보내기</a> </p>
		        </div>
   ...
   <td colspan="3">${dogBoardVO.dsboard_cont }</td>
	...
	<form action="writeComm" id="writeComm" name="formm" method="post">
		<table class="table table-condensed">
			<thead>
				<tr>
					<td colspan="3"><span class="badge">댓글  ${dsBoardCommListCnt}</span></td>
				</tr>
			</thead>
			<tbody>
				<tr>
			...
			<table class="table table-condensed" id="dscomm_go">
				<c:forEach items="${dogBoardCommList}" var="dsBoardComm" varStatus="status" >
					<input type="hidden" id="dsboard_ans_code${status.count}" value="${dsBoardComm.dsboard_ans_code }">
					<tbody>
					<tr id="dsboard_ans_code${status.count}" class="dscoment">
						...
						<button type="button" class="btn btn-default btn-xs" data-toggle="modal" data-target="#myModal2${status.count }">
	    					${ dsBoardComm.dsboard_comm_wri}</button></td>
	    				 ...
	    				 <div class="modal fade" id="myModal2${status.count }" role="dialog">
						    <div class="modal-dialog">
						      ...
						      <div class="modal-content">
						        <div class="modal-header">
						          <button type="button" class="close" data-dismiss="modal">&times;</button>
						          <h4 class="modal-title"> ${ dsBoardComm.dsboard_comm_wri} 님에게 쪽지 보내기</h4>
						        </div>
						        <div class="modal-body">
						...
						<td style="width:70%;  word-break:break-all;" id="dsBoardComm_cont${status.count}" name="dsBoardComm_cont${status.count}">${ dsBoardComm.dsboard_comm_cont}
						...
						<c:set value="${dsBoardComm.dsboard_comm_cont}" var="dsboard_comm_cont"/>
						<c:set value="${status.count}" var="indexTd"/>
						<c:set value="${dsBoardComm.dsboard_ans_code}" var="dsboard_ans_code"/>
						<c:set value="dsboard_ans_code${status.count}" var="indexTdd"/>
						
						<td>
							<a href="#" onclick="updateDsCommForm('${indexTd }', event )" id="aa">수정</a>
						/
						<%-- id=${ifShrBoardComm.ifshrboard_ans_code}; --%>
							<a href="#" onclick="deleteDsComm('${dsboard_ans_code}','${indexTdd }',event )"id="aa">삭제</a>
						</td>
						<td></td>
						</c:if>
					</tr>
		...
		<textarea rows="2" cols="150" id="dsboard_comm_contt" name="dsboard_comm_cont"></textarea>
		<input type="hidden" id="dsboard_posting_no" name="dsboard_posting_no" value="${dogBoardVO.dsboard_posting_no }">
		<input type="hidden" id="mem_nick" name="mem_nick" value="${loginUser.mem_nick}">
		<input type="button" value="댓글등록" onclick="dswriteComm_go($('#mem_nick').val(),$('#dsboard_posting_no').val(),$('#dsboard_comm_contt').val(),event)">
		</c:if>
	</form>
