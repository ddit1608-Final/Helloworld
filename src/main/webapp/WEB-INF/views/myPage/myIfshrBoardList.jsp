			...
			<tr
				...
				<td style="text-align: left:;"><select id="board_type"
					...
					style="background-color: gray; font-size: 15pt; color: white; font-family:;">
						...
						<option value="IfshrBoard" selected="selected">정보공유게시판</option>
				</select></td>
				...
				<c:when test="${total<=0}">
					<tr>
						<td width="100%" colspan="6" align="center" height="23">
							There are no registered ifshrboard.</td>
					</tr>
				</c:when>
				<c:otherwise>
					<c:forEach items="${ifshrBoardList}" var="myBoardVO">
						<tr>
							<td style="text-align: center;" colspan="2">정보공유게시판</td>
							<td style="text-align: center;"><a
								href="/world/free/freeBoardDetail.do?freeboard_posting_no=${myBoardVO.ifshrboard_posting_no}">${myBoardVO.ifshrboard_title}</a>
							</td>
							<%-- <td>${myBoardVO.freeboard_cont}</td> --%>
							<td style="text-align: center;">${myBoardVO.mem_nick}</td>
							<%-- <td>${loginUser.mem_nick}</td> --%>
							<td style="text-align: center;"><fmt:formatDate
									value="${myBoardVO.ifshrboard_wridate}" /></td>
							<td style="text-align: center;">${ myBoardVO.ifshrboard_hits}</td>
						</tr>
					</c:forEach>
					<tr>
						<td colspan="6" style="text-align: center;">${ifshrPaging}</td>
					</tr>
				</c:otherwise>
			</c:choose>
		</table>
